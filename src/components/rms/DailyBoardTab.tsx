  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-serif font-bold text-white">📢 Daily Board</h2>
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className="bg-[#F6C51B] hover:bg-yellow-400 text-[#1B1B20] font-bold text-xs uppercase px-4 py-2.5 rounded-xl flex items-center gap-2"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {items.map((a) => (
          <div key={a.id} className="bg-[#1B1B20] border border-stone-800 rounded-2xl p-4 space-y-3">
            ...
            <div className="flex items-center gap-2 text-[11px] text-stone-500">
              <Calendar className="w-3.5 h-3.5 text-[#F6C51B]" />
              <span>
            ...
            <div className="flex justify-end gap-2">
              <button onClick={() => setEditing(a)} className="p-2 bg-yellow-500/20 text-yellow-300 hover:bg-yellow-500 hover:text-black rounded-lg" title="Edit">
        ...
        {items.length === 0 && (
          <div className="col-span-2 bg-[#1B1B20] border border-stone-800 rounded-2xl p-10 text-center text-stone-500 text-xs">
      ...
      {editing && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-[#1B1B20] rounded-3xl max-w-md w-full p-6 border border-[#F6C51B] space-y-4 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-stone-800 pb-3">
              <h3 className="font-serif font-bold text-lg text-white flex items-center gap-2">
                <Megaphone className="w-5 h-5 text-[#F6C51B]" /> {editing.id ? "Edit" : "New"} Announcement
              </h3>
              <button onClick={() => setEditing(null)} className="text-stone-400 hover:text-white">✕</button>
            </div>

            <div>
              <label className="block text-xs font-bold text-yellow-200 mb-1">Title *</label>
              <input
                value={editing.title || ""}
            ...
            <div>
              <label className="block text-xs font-bold text-yellow-200 mb-1">Message *</label>
              <textarea
                rows={2}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-bold text-yellow-200 mb-1">Start Date</label>
                <input type="date" value={editing.startDate || ""} onChange={(e) => setEditing({ ...editing, startDate: e.target.value })} className="w-full bg-[#2A2A31] border border-stone-700 rounded-xl p-3 text-xs text-white" />
              </div>
              <div>
                <label className="block text-xs font-bold text-yellow-200 mb-1">End Date (auto-hides)</label>
                <input type="date" value={editing.endDate || ""} onChange={(e) => setEditing({ ...editing, endDate: e.target.value })} className="w-full bg-[#2A2A31] border border-stone-700 rounded-xl p-3 text-xs text-white" />
              </div>
            </div>

            <div className="bg-[#2A2A31] rounded-2xl p-4 border border-stone-700 space-y-3">
              <div>
                <p className="text-xs font-bold text-yellow-200 flex items-center gap-1.5"><Gift className="w-3.5 h-3.5" /> Promotion</p>
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full bg-[#1B1B20] border border-stone-700 rounded-xl p-3 text-xs text-white"
                >
                ...
                <>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input type="time" value={promotion.startTime || ""} onChange={(e) => updatePromotion({ startTime: e.target.value || null })} className="w-full bg-[#1B1B20] border border-stone-700 rounded-xl p-2.5 text-xs text-white" />
                    </div>
                    <div>
                      <label className="block text-[11px] font-bold text-stone-300 mb-1">End Time (optional)</label>
                      <input type="time" value={promotion.endTime || ""} onChange={(e) => updatePromotion({ endTime: e.target.value || null })} className="w-full bg-[#1B1B20] border border-stone-700 rounded-xl p-2.5 text-xs text-white" />
                    </div>
                  </div>
                  ...
                  {(promotion.type === "special_price" || promotion.type === "combo") && (
                    <div>
                      ...
                      <input type="number" min="0" value={promotion.specialPrice ?? 0} onChange={(e) => updatePromotion({ specialPrice: Number(e.target.value) })} className="w-full bg-[#1B1B20] border border-stone-700 rounded-xl p-2.5 text-xs text-white" />
                    </div>
                  )}
                  {promotion.type === "percentage_discount" && (
                    <div>
                      <label className="block text-[11px] font-bold text-stone-300 mb-1">Discount (%)</label>
                      <input type="number" min="0" max="100" value={promotion.discountPercent ?? 0} onChange={(e) => updatePromotion({ discountPercent: Number(e.target.value) })} className="w-full bg-[#1B1B20] border border-stone-700 rounded-xl p-2.5 text-xs text-white" />
                    </div>
                  )}
                  {promotion.type === "fixed_amount_discount" && (
                    <div>
                      ...
                      <input type="number" min="0" value={promotion.discountAmount ?? 0} onChange={(e) => updatePromotion({ discountAmount: Number(e.target.value) })} className="w-full bg-[#1B1B20] border border-stone-700 rounded-xl p-2.5 text-xs text-white" />
                    </div>
                  )}
                  {promotion.type === "buy_x_get_y_discounted" && (
                    <div>
                      ...
                      <input type="number" min="0" max="100" value={promotion.getDiscountPercent ?? 0} onChange={(e) => updatePromotion({ getDiscountPercent: Number(e.target.value) })} className="w-full bg-[#1B1B20] border border-stone-700 rounded-xl p-2.5 text-xs text-white" />
                    </div>
                  )}
                  ...
                  <div className="space-y-2">
                    ...
                    {promotion.items.map((promotionItem, index) => (
                      <div key={index} className="rounded-xl border border-stone-700 bg-[#1B1B20] p-3 space-y-2">
                        <div className="flex gap-2">
                          <select value={promotionItem.menuItemId || ""} onChange={(e) => updatePromotionItem(index, { menuItemId: Number(e.target.value) })} className="min-w-0 flex-1 bg-[#2A2A31] border border-stone-700 rounded-lg px-2 py-2 text-xs text-white">
                        ...
                        <div className="flex items-center justify-between gap-3">
                          <label className="flex items-center gap-2 text-[11px] font-bold text-stone-300">
                            Qty
                            <input type="number" min="1" max="100" value={promotionItem.quantity} onChange={(e) => updatePromotionItem(index, { quantity: Number(e.target.value) })} className="w-16 bg-[#2A2A31] border border-stone-700 rounded-lg px-2 py-1.5 text-xs text-white" />
                          </label>
                          {needsBuyGetRoles && (
                            <label className="flex items-center gap-2 text-[11px] font-bold text-yellow-200">
                              <span>Customer gets</span>
                              <select value={promotionItem.role || "buy"} onChange={(e) => updatePromotionItem(index, { role: e.target.value as "buy" | "get" })} className="bg-[#2A2A31] border border-stone-700 rounded-lg px-2 py-1.5 text-xs text-white">
                  ...
                  {canAddPromotionItem && (
                    <button type="button" onClick={() => updatePromotion({ items: [...promotion.items, { ...defaultItem(), ...(needsBuyGetRoles ? { role: "get" as const } : {}) }] })} disabled={menuItems.length === 0} className="w-full border border-dashed border-[#F6C51B]/60 text-yellow-200 hover:bg-[#F6C51B]/10 text-xs font-bold py-2 rounded-xl disabled:opacity-40">
            ...
            <div className="bg-[#2A2A31] rounded-2xl p-4 border border-stone-700 space-y-2">
              <p className="text-xs font-bold text-yellow-200">Image (optional)</p>
              ...
              <label className="flex items-center justify-center gap-2 w-full bg-[#F6C51B] hover:bg-yellow-400 text-[#1B1B20] font-extrabold text-xs py-2.5 rounded-xl cursor-pointer">
            ...
            <div>
              <label className="block text-xs font-bold text-yellow-200 mb-1">Priority (lower shows first)</label>
              <input type="number" value={editing.priority ?? 0} onChange={(e) => setEditing({ ...editing, priority: Number(e.target.value) })} className="w-full bg-[#2A2A31] border border-stone-700 rounded-xl p-3 text-xs text-white" />
            </div>

            <button onClick={save} disabled={saving || !editing.title} className="w-full bg-gradient-to-r from-[#F6C51B] to-yellow-500 text-[#1B1B20] font-black text-sm uppercase py-3.5 rounded-xl disabled:opacity-40">
